...
3. Check for project conventions: AGENTS.md, docs/ARCHITECTURE.md, docs/CONVENTIONS.md

### For plan mode

1. The plan content is already available from the `@` file reference
2. Search for existing code related to the plan (patterns, similar implementations, relevant modules)
3. Check for AGENTS.md, docs/ARCHITECTURE.md for project context
...
After presenting results, based on severity:

- **BLOCK/HIGH** (any count) -> must fix, then re-review
- **MEDIUM** (any count) -> must fix (no follow-up tickets exist). Recommend re-review if 3+
- **LOW only** -> document as `// TODO:` in code, no fix needed now

Suggest next steps:
- If BLOCK/HIGH: "Must fix before push: [list]. Re-review after fixes."
- If 3+ MEDIUM: "Must fix these before push: [list]. Recommend re-review."
- If 1-2 MEDIUM: "Fix these before push: [list]"
- If LOW only: "No blockers. Add `// TODO:` for these: [list]"

### Re-Review Rounds (max 2)

After fixes, re-dispatch `@check` to verify:
- Max 2 re-review rounds
- After 2 rounds: BLOCK/HIGH remaining -> STOP, report to user. MEDIUM/LOW -> push + TODO.